...
public class OthelloState {
    ...
    // Checks which players controls corners and adjusts score accordingly
    public int corners(){
        ...
        if(board[0][c]==PLAYER1) score++;
        if (board[0][c]==PLAYER2) score--;

        // Check bottom left corner
        if(board[c][0]==PLAYER1) score++;
        if (board[c][0]==PLAYER2) score--;

        // Check bottom right corner
        if(board[c][c]==PLAYER1) score++;
        if (board[c][c]==PLAYER2) score--;

        return 0;
    }

    // Checks which player has more moves and adjusts score accordingly
    public int mobililty(){

        int score = 0;
        score += this.generateMoves(0).size();
        score -= this.generateMoves(1).size();
        return score;
    }
    ...
    public int eval(){
        ...
        int mobility = this.mobililty();

        return score+corners+mobility;
    }
